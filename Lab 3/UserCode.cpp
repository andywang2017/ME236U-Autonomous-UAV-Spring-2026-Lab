...
//  motorCommand4 -> located at body -x +y
  if (lastMainLoopInputs.joystickInput.buttonBlue){
    float desForce = 0.0704f;
    float desSpd = speedFromForce(desForce);
    outVals.motorCommand1 = pwmCommandFromSpeed(desSpd);
    outVals.motorCommand2 = pwmCommandFromSpeed(desSpd);
    outVals.motorCommand3 = pwmCommandFromSpeed(desSpd);
    outVals.motorCommand4 = pwmCommandFromSpeed(desSpd);
//    float desForce = 0.04f;
//    outVals.motorCommand1 = speedFromForce(desForce);
//    outVals.motorCommand2 = speedFromForce(desForce);
//    outVals.motorCommand3 = speedFromForce(desForce);
//    outVals.motorCommand4 = speedFromForce(desForce);
  }
  else{
